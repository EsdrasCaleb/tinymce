...
var toc = (function () {
  ...
  var $_hvc6arkji394kkf = { create: create };

  var tocId = $_hvc6arkji394kkf.create('mcetoc_');
  ...
  var readHeaders = function (editor) {
    var tocClass = $_3zdhvarjji394kkf.getTocClass(editor);
    var headerTag = $_3zdhvarjji394kkf.getTocHeader(editor);
    var selector = generateSelector($_3zdhvarjji394kkf.getTocDepth(editor));
  ...
  var generateTocHtml = function (editor) {
    ...
    return '<div class="' + editor.dom.encode($_3zdhvarjji394kkf.getTocClass(editor)) + '" contenteditable="false">' + html + '</div>';
  };
  var generateTocContentHtml = function (editor) {
    ...
    html += generateTitle($_3zdhvarjji394kkf.getTocHeader(editor), global$2.translate('Table of Contents'));
    for (i = 0; i < headers.length; i++) {
      h = headers[i];
      h.element.id = h.id;
  ...
  var register = function (editor) {
    editor.addCommand('mceInsertToc', function () {
      $_508drwrfji394kk8.insertToc(editor);
    });
    editor.addCommand('mceUpdateToc', function () {
      $_508drwrfji394kk8.updateToc(editor);
    });
  };
  var $_lwy00reji394kk6 = { register: register };

  var setup = function (editor) {
    var $ = editor.$, tocClass = $_3zdhvarjji394kkf.getTocClass(editor);
  ...
  var $_3fwmlerlji394kkg = { setup: setup };

  var toggleState = function (editor) {
    return function (e) {
      var ctrl = e.control;
      editor.on('LoadContent SetContent change', function () {
        ctrl.disabled(editor.readonly || !$_508drwrfji394kk8.hasHeaders(editor));
      });
    };
  };
  var isToc = function (editor) {
    return function (elm) {
      return elm && editor.dom.is(elm, '.' + $_3zdhvarjji394kkf.getTocClass(editor)) && editor.getBody().contains(elm);
    };
  };
  ...
  var $_3tttl0rmji394kki = { register: register$1 };

  global.add('toc', function (editor) {
    $_lwy00reji394kk6.register(editor);
    $_3tttl0rmji394kki.register(editor);
    $_3fwmlerlji394kkg.setup(editor);
  });
  function Plugin () {
  }
